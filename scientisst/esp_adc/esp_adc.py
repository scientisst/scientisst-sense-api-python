...
def calculate_voltage_linear(adc_reading, coeff_a, coeff_b):
    ...
    return (
        int(((coeff_a * adc_reading) + LIN_COEFF_A_ROUND) / LIN_COEFF_A_SCALE) + coeff_b
    )
...
def esp_adc_cal_raw_to_voltage(adc_reading, chars):
    adc_reading = adc_reading << (ADC_WIDTH_BIT_12 - chars.bit_width)
    if adc_reading > ADC_12_BIT_RES - 1:
        adc_reading = ADC_12_BIT_RES - 1

    if chars.atten == ADC_ATTEN_DB_11 and adc_reading >= LUT_LOW_THRESH:
        #
        lut_voltage = calculate_voltage_lut(
            adc_reading, chars.vref, chars.low_curve, chars.high_curve
        )

        if adc_reading <= LUT_HIGH_THRESH:
            linear_voltage = calculate_voltage_linear(
                adc_reading, chars.coeff_a, chars.coeff_b
            )

            return interpolate_two_points(
                linear_voltage,
                lut_voltage,
                LUT_ADC_STEP_SIZE,
